...
namespace Scine {
namespace Readuct {
using namespace ElementaryStepOptimization::CostBasedOptimization;
namespace Tests {
...
class CommonCostTerms : public Test {
 ...
 protected:
  void SetUp() override {
    ...
    Utils::BSplines::InterpolationGenerator generator(randomPoints);
...
double CommonCostTerms::squaredNorm(const Utils::BSplines::BSpline& s, double u) const {
...
Eigen::MatrixXd CommonCostTerms::numericalSquaredNormDerivatives(double u) const {
  ...
  for (int i = 0; i < numberControlPoints; ++i) {
    ...
    for (int j = 0; j < randomBSpline.getDim(); ++j) {
      ...
      Utils::BSplines::BSpline splineP{knotVector, controlPointsP, randomBSpline.getDegree()};
...
double CommonCostTerms::dotProductOfGradientsAndSplineDerivative(const Utils::BSplines::BSpline& s, double u) const {
...
double CommonCostTerms::dotProductOfPositionAndSplineDerivative(const Utils::BSplines::BSpline& s, double uPos,
                                                                double uTan) const {
...
double CommonCostTerms::dotProductOfPositions(const Utils::BSplines::BSpline& s, double uA, double uB) const {
...
double CommonCostTerms::squareDistance(const Utils::BSplines::BSpline& s, double uA, double uB) const {
  auto positionA = s.evaluate(uA);
  auto positionB = s.evaluate(uB);
  return (positionA - positionB).squaredNorm();
}

double CommonCostTerms::squareDistanceToPoint(const Utils::BSplines::BSpline& s, double u, const Eigen::VectorXd& p) const {
...
double CommonCostTerms::derivativeOfSquareTangentNorm(const Utils::BSplines::BSpline& s, double u) const {
